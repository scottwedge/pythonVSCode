...
suite('DataScience Native Editor', () => {
    ...
    suite('Editor Keyboard tests', () => {
        ...
        test('Toggle markdown and code', async () => {
            clickCell(0);

            simulateKeyPressOnCell(0, { code: 'm' });
            // In markdown mode, monaco editors is not rendered, and output component is.
            assert.equal(wrapper.find(NativeCell).first().find(MonacoEditor).length, 0);
            assert.equal(wrapper.find(NativeCell).first().find(CellOutput).length, 1);

            const cellWrapper = wrapper.find(NativeCell).first().find('.cell-wrapper').first();
            cellWrapper.simulate('keyDown', {key: 'y', code: 'y'.charCodeAt(0) });

            // In code mode, monaco editors and no output components are rendered (unless there's an output).
            assert.equal(wrapper.find(NativeCell).first().find(MonacoEditor).length, 1);
            assert.equal(wrapper.find(NativeCell).first().find(CellOutput).length, 0);
        });
    });
});
