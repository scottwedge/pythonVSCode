...
import { ICellViewModel, IMainState, testState } from '../../datascience-ui/interactive-common/mainState';
import { noop } from '../core';

// When testing, initialize the test state.
testState.generateTestState = generateTestState;
...
function createEmptyCell(id: string | undefined, executionCount: number | null): ICell {
    return {
        data:
        {
...
function createEditableCellVM(executionCount: number): ICellViewModel {
    return {
...
function extractInputText(inputCell: ICell, settings: IDataScienceSettings | undefined): string {
    let source: string[] = [];
...
function generateCells(filePath: string, repetitions: number): ICell[] {
